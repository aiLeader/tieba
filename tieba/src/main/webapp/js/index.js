...
function IndexListNote(url){
	$.post(url,function(data){
		...
		for(var i=0;i<data.rows.length;i++){
			...
			+'</p><p>'+data.rows[i].ncontent+'</p><p><span class="glyphicon glyphicon-user"></span><a href="#" style="padding-right:30px">'+data.rows[i].user.uname+'</a>'
			...
			+'</p><p><a class="btn" href="page/noteDetail.jsp">进入帖子 »</a></p></div>');
		}
